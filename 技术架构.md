...
- **多方式登录**：
  ...
  - **验证码登录**：支持手机号 / 邮箱 + 验证码
  - **社交登录**：GitHub, Google, Apple, QQ, WeChat
...
- **OAuth 提供商**：GitHub、Google、Apple、qq、wechat（通过 `arctic` 库，类型安全的 OAuth 2.0 / OIDC 客户端）
...
| `qq_id`         | `VARCHAR(50)`  | UNIQUE                        | QQ OAuth ID                      |
| `wechat_id`     | `VARCHAR(50)`  | UNIQUE                        | WeChat OAuth ID                  |
...
>
> 1. **基础排版**：实现段落缩进与首字下沉。
> 2. **数学公式**：输入勾股定理 `a^2 + b^2 = c^2`。
> 3. **矩阵排版**：使用 `bmatrix` 环境排版 3x3 矩阵。
> 4. **TikZ 绘图**：绘制一个带有箭头的流程图节点。
...
| `slug`         | `VARCHAR(100)` | UNIQUE        | URL 别名（用于生成语义化链接） |
...
_本文档由 Latexia 架构团队维护，如有变更请同步更新本文档并在 CHANGELOG.md 中记录。_
